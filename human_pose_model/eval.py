...
def eval(model, split, seq_length, n_cpu, disp):
    ...
    df = pd.read_pickle('data/val_split_{}.pkl'.format(split))
    ...
    for i, sample in enumerate(data_loader):
        ...
        while batch * seq_length < images.shape[1]:
            if (batch + 1) * seq_length > images.shape[1]:
                ...
                landmarks_batch = landmarks[:, batch * seq_length:, :]
            else:
                image_batch = images[:, batch * seq_length:(batch + 1) * seq_length, :, :, :]
                landmarks_batch = landmarks[:, batch * seq_length:(batch + 1) * seq_length, :]
            logits = model(image_batch.cuda(), landmarks_batch.cuda())
            if batch == 0:
